...
import atexit
import shutil
import threading, time, webbrowser  # NEU
...
# =========================
# Auto-Open in neuem Fenster (OS & Browser wählbar)
# =========================

# Standardwerte können per Env überschrieben werden
APP_PORT = int(os.environ.get("APP_PORT", "5100"))
APP_HOST = os.environ.get("APP_HOST", "127.0.0.1")
APP_AUTO_OPEN = os.environ.get("APP_AUTO_OPEN", "true").lower() in ("1", "true", "yes", "on")
APP_BROWSER = os.environ.get("APP_BROWSER", "chrome").lower()  # chrome|edge|safari|system


def _open_url_new_window(url: str, browser: str = "chrome", width: int = 1200, height: int = 800, x: int = 120,
                         y: int = 80):
  plat = sys.platform

  # macOS – Chrome-Fenstergröße via AppleScript setzen (zuverlässig)
  if plat.startswith("darwin") and browser == "chrome":
    try:
      script = f'''
              tell application "Google Chrome"
                  ...
                  -- neues Fenster erzeugen und URL setzen
                  make new window
                  set URL of active tab of front window to "{url}"
                  -- Fensterposition und -größe setzen: {{left, top, right, bottom}}
      ...
      import webbrowser
      webbrowser.get("open -na 'Google Chrome' --args --new-window %s").open(url)
      return

  # Windows – Flags funktionieren i.d.R. zuverlässig
  if plat.startswith("win"):
    import shutil, webbrowser, os
    chrome_paths = [
    ...
    for p in chrome_paths:
      if os.path.exists(p):
        webbrowser.get(f'"{p}" --new-window --window-size={width},{height} %s').open(url)
        return
    webbrowser.open_new(url)
    return

  # Linux/sonstige – versuchen, chrome/chromium mit --window-size zu finden
  import shutil, webbrowser
  candidates = ["google-chrome", "chrome", "chromium-browser", "chromium"]
  for exe in candidates:
    exe_path = shutil.which(exe)
    if exe_path:
      webbrowser.get(f'"{exe_path}" --new-window --window-size={width},{height} %s').open(url)
      return
  webbrowser.open_new(url)


if __name__ == "__main__":
    # Browser automatisch in neuem Fenster öffnen (Thread, damit Flask zuerst hochfährt)
    if APP_AUTO_OPEN:
        def _delayed_open():
            # kleinen Moment warten, bis der Server antwortet
            time.sleep(1.0)
